''' Hollis Brookline Middle School Greenhouse sensor data project.
    ...
    Silicon Labs Tunderboards for temperature, humidity and light level (lux).

    The Thunderboards are configured to send data every few minutes.
...
# BluePy is pre-installed on the RPi3.
...
if __name__ == "__main__":
    ...
    try:
        ...
        u=utility()
        u.usage()  # if executed without being root (sudo) this is executed.
        sys.exit(1)
    tags=[]
    ...
    for dev in devs:
        ...
        for (adtype,desc,value) in dev.getScanData():
            ...
            if "Thunder" in value:
                print "found TB sensor {}".format(dev.addr)
                tags.append(SensorTag(dev.addr))
    print "Found {} ThunderBoards.".format(len(tags))
    if len(tags)<1:
        print "No Sensor found. Exiting"
        sys.exit(3)
