...
if __name__ == "__main__":
    ...
    for i in csv_list:
        ...
        os.system('gnuplot gengraph.plt > ../web/pix/{}.png'.format(i[:-4]))
        os.system('cat {} > genplot.dat'.format(i))                 # plot all the data which is a pretty busy graph
        os.system('gnuplot gengraph.plt > ../web/pix/{}All.png'.format(i[:-4]))
    os.system('rm -f genplot.dat')
    senhtml=open("../web/sensors.html",'w',1)
    ...
    for i in csv_list:
        senhtml.write('<h1>Sensor <a href="../software/{0}">{0}</a> last 24 hours</h2>\n'.format(i))
